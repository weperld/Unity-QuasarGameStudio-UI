...
using UnityEditor;
using UnityEngine;
using In_Game = DefineName.Audio_InGame.BGM;
using Out_Game = DefineName.Audio_OutGame.BGM;
using Visual_Story = DefineName.Audio_Story.BGM;

[CustomPropertyDrawer(typeof(PlayBGMBehaviour))]
public class PlayBGMDrawer : PropertyDrawer
{
    private string[] arr_MainCategory = new string[] { nameof(In_Game), nameof(Out_Game), nameof(Visual_Story) };
    private string[] arr_BGM;

    GUIContent gc_Label = new GUIContent("BGM 추가 필요 시 개발자에게 요청하여,\n" +
        "DefineName_Audio에 카테고리에 맞춰 파일명 추가 필요");

    private readonly string categoryLabel = "BGM Audio Category";
    private readonly string audioLabel = "BGM Audio Selection";
    ...
    {
        var volumeCurveProp = property.FindPropertyRelative("volumeCurve");
        ...
        if (arr_BGM.Length > 0)
        {
            ...
            audioIndex = EditorGUI.Popup(rect, audioLabel, audioIndex, arr_BGM);
            audioNameProp.stringValue = arr_BGM[audioIndex];

            rect.y += EditorGUIUtility.singleLineHeight;
            EditorGUI.PropertyField(rect, volumeCurveProp);
